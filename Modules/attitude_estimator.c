...
void quarternion_init(const vec3f_t* acc, const vec3f_t* mag, stateAtt_t* stateAtt)
{
	...
	xh = mag->y*cos(roll)+mag->x*sin(roll)*sin(pitch)-mag->z*cos(pitch)*sin(roll);		 
	yh = mag->x*cos(pitch)+mag->z*sin(pitch);
...
void margUpdate(float w_x, float w_y, float w_z, float a_x, float a_y, float a_z, float m_x, float m_y, float m_z)
{
...
void imuUpdate(float w_x, float w_y, float w_z, float a_x, float a_y, float a_z)
{
	// Local system variables
	float norm; // vector norm
